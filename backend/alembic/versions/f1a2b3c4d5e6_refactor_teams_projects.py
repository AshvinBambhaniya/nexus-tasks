...
def upgrade() -> None:
    ...
    if bind.engine.name == 'postgresql':
        op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'teamrole') THEN CREATE TYPE teamrole AS ENUM ('ADMIN', 'MEMBER'); END IF; END $$;")
        op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'projectrole') THEN CREATE TYPE projectrole AS ENUM ('ADMIN', 'MEMBER', 'VIEWER'); END IF; END $$;")
    ...
    op.create_table('team_members',
        ...
        sa.Column('role', postgresql.ENUM('ADMIN', 'MEMBER', name='teamrole', create_type=False), nullable=False),
    ...
    op.create_table('project_members',
        ...
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('role', postgresql.ENUM('ADMIN', 'MEMBER', 'VIEWER', name='projectrole', create_type=False), nullable=False),
...
def downgrade() -> None:
    ...
    op.create_foreign_key('tasks_workspace_id_fkey', 'tasks', 'workspaces', ['workspace_id'], ['id'], ondelete='CASCADE')
