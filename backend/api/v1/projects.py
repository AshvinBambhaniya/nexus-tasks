...
def validate_project_access(
    project_id: int, db: Session, user_id: int, require_admin: bool = False
) -> ProjectMember:
    ...
    team_access = (
        ...
        .filter(ProjectTeam.project_id == project_id, TeamMember.user_id == user_id)
        .first()
    )
    ...
    if ws_member and ws_member.role == WorkspaceRole.ADMIN:
        return ProjectMember(
            project_id=project_id, user_id=user_id, role=ProjectRole.ADMIN
        )
...
def create_project(
    ...
    if not ws_member or ws_member.role != WorkspaceRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only Workspace Admins can create projects"
        )
    ...
    member = ProjectMember(
        project_id=new_project.id, user_id=current_user.id, role=ProjectRole.ADMIN
    )
    db.add(member)
    db.commit()
...
def add_project_member(
    ...
    new_member = ProjectMember(
        project_id=project_id, user_id=user_to_add.id, role=member_data.role
    )
    db.add(new_member)
    db.commit()
...
def add_project_team(
    ...
    if team.workspace_id != project.workspace_id:
        raise HTTPException(
            status_code=400, detail="Team must belong to the same workspace"
        )
    ...
    new_link = ProjectTeam(project_id=project_id, team_id=team_data.team_id)
    db.add(new_link)
    db.commit()
    db.refresh(new_link)
