...
from models.workspace import Workspace, WorkspaceMember, WorkspaceRole, WorkspaceType
...
def create_workspace(
    ...
    member = WorkspaceMember(
        workspace_id=new_ws.id, user_id=current_user.id, role=WorkspaceRole.ADMIN
    )
    db.add(member)
    db.commit()
...
def validate_workspace_access(
    workspace_id: int, db: Session, user_id: int
) -> WorkspaceMember:
    member = (
...
def invite_member(
    ...
    new_member = WorkspaceMember(
        workspace_id=workspace_id, user_id=user_to_add.id, role=WorkspaceRole.MEMBER
    )
    db.add(new_member)
    db.commit()
