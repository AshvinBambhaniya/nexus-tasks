...
from schemas.team import TeamCreate, TeamMemberAdd, TeamMemberResponse, TeamResponse

router = APIRouter()


def validate_team_access(
    team_id: int, db: Session, user_id: int, require_admin: bool = False
) -> TeamMember:
    """
    ...
    if ws_member and ws_member.role == WorkspaceRole.ADMIN:
        return TeamMember(
            team_id=team_id, user_id=user_id, role=TeamRole.ADMIN
        )  # Mock object for auth success
...
def create_team(
    ...
    if not ws_member or ws_member.role != WorkspaceRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only Workspace Admins can create teams"
        )
    ...
    member = TeamMember(
        team_id=new_team.id, user_id=current_user.id, role=TeamRole.ADMIN
    )
    db.add(member)
    db.commit()
...
def list_team_projects(
    ...
    projects = (
        ...
        .filter(ProjectTeam.team_id == team_id)
        .all()
    )
...
def add_team_member(
    ...
    exists = (
        db.query(TeamMember)
        .filter(TeamMember.team_id == team_id, TeamMember.user_id == user_to_add.id)
        .first()
    )
    if exists:
    ...
    new_member = TeamMember(
        team_id=team_id, user_id=user_to_add.id, role=member_data.role
    )
    db.add(new_member)
    db.commit()
...
def list_team_members(
    ...
    return [
        {"user_id": m.user_id, "role": m.role, "email": m.user.email} for m in members
    ]
