...
<template>
  <div>
    <div v-if="!activeWorkspace" class="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 bg-gray-50/50 p-16 text-center">
      <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-gray-100">
        <UserIcon class="h-8 w-8 text-gray-400" />
      </div>
      <h3 class="text-lg font-bold text-gray-900">No Workspace Selected</h3>
      <p class="mt-1 max-w-xs text-sm text-gray-500 text-pretty">Select a workspace from the dashboard to manage its members and permissions.</p>
    </div>

    <div v-else class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
      <!-- Invite Section -->
      <section>
        <div class="mb-4">
          <h2 class="text-lg font-semibold text-gray-900">Invite New Members</h2>
          <p class="text-sm text-gray-500">Expand your team by inviting collaborators to this workspace.</p>
        </div>
        
        <UiBaseCard class="border-gray-100 shadow-sm transition-shadow hover:shadow-md">
          <div class="p-8">
            <form @submit.prevent="handleInvite" class="flex flex-col gap-4 sm:flex-row">
              <div class="relative flex-1">
                <Mail class="absolute top-1/2 left-3.5 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <UiBaseInput
              ...
              <UiBaseButton type="submit" :disabled="isInviting" class="h-11 px-8 shadow-sm active:scale-95">
      ...
      <section>
        <div class="mb-4 flex items-end justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500">A list of all users with access to this environment.</p>
          </div>
          <div class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md">
        ...
        <UiBaseCard class="overflow-hidden border-gray-100 shadow-sm">
          ...
          <div v-if="isLoading" class="flex flex-col items-center justify-center p-20">
            ...
            <p class="mt-4 text-sm font-medium text-gray-400">Loading directory...</p>
          </div>
          ...
          <div v-else class="divide-y divide-gray-50">
            ...
            >
              <div class="flex items-center gap-5">
                <div class="relative">
                  ...
                  />
                  <div v-if="member.role === 'ADMIN'" class="absolute -right-1 -bottom-1 flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-white ring-2 ring-white">
                    <Shield class="h-3 w-3" />
                  </div>
                </div>
                <div>
                  <div class="flex items-center gap-2">
                    <span class="font-bold text-gray-900">{{ member.user.full_name || member.user.email.split("@")[0] }}</span>
                    <UiBaseBadge v-if="member.user.id === userStore.userData?.id" class="bg-gray-100 text-[9px] font-black text-gray-500 uppercase">You</UiBaseBadge>
                  </div>
                  <div class="text-sm text-gray-500 transition-colors group-hover:text-blue-600/70">{{ member.user.email }}</div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-6">
                <UiBaseBadge
                  ...
                  :class="[
                    ...
                    member.role === 'ADMIN' ? 'bg-blue-600 ring-blue-700' : 'bg-white text-gray-500 ring-gray-100'
                  ]"
                >
                ...
                <div class="flex w-10 justify-center">
                  <UiBaseButton
                    v-if="member.role !== 'ADMIN' && member.user.id !== userStore.userData?.id"
                    variant="ghost"
                    size="sm"
            ...
            <div v-if="!filteredMembers || filteredMembers.length === 0" class="flex flex-col items-center justify-center p-20 text-center">
              ...
              <p class="mt-1 text-sm text-gray-500">Try adjusting your search or inviting new team members.</p>
            </div>
          </div>
        </UiBaseCard>
      </section>
