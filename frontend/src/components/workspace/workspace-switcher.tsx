      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-full overflow-hidden rounded-md border border-gray-200 bg-white shadow-lg">
          <div className="max-h-[300px] overflow-auto p-1">
            {workspaces.length === 0 ? (
              <div className="px-2 py-2 text-sm text-gray-500">No workspaces found.</div>
            ) : (
              workspaces.map((workspace) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeWorkspace?.id === workspace.id && "bg-blue-50 text-blue-600"
                  )}
                >
          ...
          <div className="border-t border-gray-100 p-1">
            <button
              onClick={() => setOpen(false)} // Placeholder for "Create Workspace"
