...
export function TaskIssueItem({ task, projectId }: TaskIssueItemProps) {
  ...
  return (
    <div className="flex items-start gap-3 p-4 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0 group">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <div className="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
          <span>#{task.id}</span>
          <span>opened {formatDistanceToNow(new Date(task.created_at))} ago</span>
          {task.assignee && (
             <div className="flex items-center gap-1">
                <Avatar fallback={task.assignee.email[0].toUpperCase()} className="h-4 w-4 text-[8px]" />
                <span className="hover:text-blue-600 cursor-pointer">{task.assignee.email.split("@")[0]}</span>
             </div>
          )}
        </div>
      </div>

      <div className="shrink-0 flex items-center gap-4 text-gray-400">
