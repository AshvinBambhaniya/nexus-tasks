...
export default function DashboardPage() {
  ...
  const workspaceTasks = myTasks.filter((t) =>
    workspaceProjectIds.has(t.project_id)
  );
          ...
          {projectsLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {projects.slice(0, 4).map((project) => (
                ...
                >
                  <Card className="group h-full cursor-pointer border-gray-200 p-5 transition-shadow hover:border-blue-200 hover:shadow-md">
                    ...
                    <h3 className="mb-1 font-semibold text-gray-900 transition-colors group-hover:text-blue-700">
                      {project.name}
                    </h3>
                    ...
                    <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-4 text-xs text-gray-500">
                      ...
                      <ArrowRight className="h-3 w-3 transform opacity-0 transition-opacity group-hover:translate-x-1 group-hover:opacity-100" />
                    </div>
                  </Card>
                </Link>
              ))}
          ...
          <div className="space-y-3">
            {tasksLoading ? (
              ...
              recentTasks.map((task) => (
                <div
                  key={task.id}
                  className="group flex items-start gap-3 rounded-lg border border-gray-200 bg-white p-3 transition-all hover:border-blue-200 hover:shadow-sm"
                >
                  <div
                    className={cn(
            ...
            {workspaceTasks.length > 5 && (
              <Link
                href="/inbox"
                className="mt-2 block text-center text-xs text-gray-500 hover:text-gray-900"
              >
