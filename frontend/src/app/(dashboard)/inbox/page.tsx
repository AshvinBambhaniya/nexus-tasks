    ...
    <div className="flex h-full flex-col space-y-6 overflow-hidden p-6">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pr-2">
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => <InboxTaskCard key={task.id} task={task} />)
        )}
      </div>
    </div>
  );
...
function InboxTaskCard({ task }: { task: TaskWithProject }) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
